 ...
 * `.env.local` fora do git, e o fluxo de release não passava valor nenhum.
 *
 ...
 * de impedir.
 */
function assertCspAllows(origin: string): void {
  const csp: unknown = tauriConfig.app.security.csp;
  ...
  if (!csp.includes(host)) {
    throw new Error(
      `O CSP em src-tauri/tauri.conf.json não permite ${host}.\n` +
        'O aplicativo abriria normalmente e toda requisição seria bloqueada em silêncio.\n' +
        `Acrescente http://${host}, ws://${host} e o ws:// do LiveKit ao connect-src.`,
    );
  }
}
