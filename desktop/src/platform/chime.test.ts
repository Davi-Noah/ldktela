...
describe('o aviso de uma tela que entra ou sai', () => {
  it('não toca para a própria transmissão', () => {
    expect(shouldChime({ publisherId: 'ana', selfId: 'ana', now: 1000, lastAt: null })).toBe(false);
  });

  it('toca para a tela de outra pessoa', () => {
    expect(shouldChime({ publisherId: 'ana', selfId: 'bruno', now: 1000, lastAt: null })).toBe(
      true,
    );
  });
  ...
  it('toca mesmo sem saber quem somos, porque calar seria pior', () => {
    ...
    expect(shouldChime({ publisherId: 'ana', selfId: undefined, now: 1000, lastAt: null })).toBe(
      true,
    );
  });
  ...
  it('colapsa uma rajada num aviso só', () => {
    ...
    expect(shouldChime({ publisherId: 'ana', selfId: 'eu', now: 1200, lastAt: 1000 })).toBe(false);
  });

  it('volta a tocar quando a rajada passou', () => {
    expect(shouldChime({ publisherId: 'ana', selfId: 'eu', now: 3000, lastAt: 1000 })).toBe(true);
  });
  ...
  it('não quebra onde não existe áudio', () => {
    ...
    expect(() => {
      chimeForShare('start', 'ana', 'eu');
      chimeForShare('stop', 'ana', 'eu');
    }).not.toThrow();
  });
});
